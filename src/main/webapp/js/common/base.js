...
var Manager={
	...
	checkLogin:function(data){//检查是否登录超时
		if(data.logoutFlag){
			...
			Manager.alert('提示',"登录超时,点击确定重新登录.",'error',Manager.toLogin);
			return false;
		}
		return true;
	},
			 	...
			 	error:function(response, textStatus, errorThrown){
			 		try{
				 		...
				 		if(!Manager.checkLogin(data)){
					 		...
					 		Manager.alert('提示', data.error || "请求出现异常,请联系管理员",'error');
					 	}
			 		}catch(e){
			 			alert(e);
			 			Manager.alert('提示',"请求出现异常,请联系管理员1",'error');
			 		}
			 	},
			 	complete:function(){
	...
	submitForm:function(form,callback,dataType){
         ...
         <!--未知功能块 -->
            ...
            if(KindEditor) {
				for ( instance in KindEditor.instances ) {
					KindEditor.instances[instance].sync();
				}
			}

			 Manager.ajaxSubmit(form,option);
	},
	saveForm:function(form,callback){
		if(form.form('validate')){
			Manager.progress('Please waiting','Saving...');
			//ajax提交form
			Manager.submitForm(form,function(data){
				Manager.closeProgress();
			 	if(data.success == "true"){
			 		if(callback){
				       	callback(data);
				    }
			 		Manager.alert('提示','保存成功.','info');
		        }else{
		        	if(data.error) {
		        		Manager.alert('提示',data.error,'error');  
		        	} else {
		        		Manager.alert('提示','Save Error','error');  
		        	}
	...
	getById:function(url,option,callback){
		Manager.progress();
		Manager.ajaxJson(url,option,function(data){
			Manager.closeProgress();
			if(data){
				if(callback){
			       	callback(data);
