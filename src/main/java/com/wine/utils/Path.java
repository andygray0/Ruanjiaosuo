...
*/
public class Path {
    public static StringBuffer getWebsitePath() {
        try {
            String a = getPathFromClass(Path.class);
            StringBuffer path = new StringBuffer(a);
            path.replace(a.indexOf(File.separator + "WEB-INF" + File.separator + "classes"),a.length(),"");//得到虚拟空间上的网站路径
            return path;
        }
        catch (Exception e) {
            e.printStackTrace();
